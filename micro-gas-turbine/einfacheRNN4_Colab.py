...
# VERSION: RNN4 - Mit 4 LSTM SCHICHTEN (statt 3) - TIEFERES NETZWERK
# EXPERIMENTE: Generalisiert ein tieferes Netzwerk besser?
# ============================================================================

# SCHRITT 0: SETUP FÜR GOOGLE COLAB
print("=" * 80)
print("SCHRITT 0: SETUP FÜR GOOGLE COLAB - RNN4 MIT SCHEDULER (4 LSTM LAGEN)")
print("=" * 80)
...
print("\n" + "=" * 80)
print("SCHRITT 4: RNN-MODELL (4 LSTM LAGEN)")
print("=" * 80)

class EinfachesRNNModell(nn.Module):
    def __init__(self, eingabedimension=1, verstecktedimension=32, anzahl_lstm_schichten=4, ausgabedimension=1):
        super(EinfachesRNNModell, self).__init__()
        # ← RNN4 EXPERIMENTE: 4 LSTM SLOJA (statt 3)
        ...
        self.lstm4 = nn.LSTM(input_size=32, hidden_size=32, batch_first=True)  # ← NOVI SLOJ!
    ...
    def forward(self, x):
        ...
        ausgabe_lstm4, _ = self.lstm4(ausgabe_lstm3)  # ← NOVI SLOJ U FORWARD!
        letzter_wert = ausgabe_lstm4[:, -1, :]  # ← UZMI IZ LSTM4
...
print("🚀 Training gestartet (RNN4 mit 4 LSTM Lagen)...\n")
...
for epoche in tqdm(range(anzahl_epochen), desc="Epochen", unit="Epoche"):
    ...
    if durchschnittlicher_verlust < bester_verlust:
        ...
        geduld_zaehler = 0
        torch.save(modell.state_dict(), 'einfachesRNN4.pth')
...
files.download('einfachesRNN4.pth')
print("✅ Modell 'einfachesRNN4.pth' heruntergeladen!")

print("\n" + "=" * 80)
print("FERTIG!")
print("=" * 80)
print("Modell ist trainiert und heruntergeladen!")
print("\n🎉 RNN4 (Tieferes Netzwerk mit 4 LSTM Lagen) ist abgeschlossen!")
print("Vergleiche RMSE mit RNN2 um zu sehen ob tieferes Netzwerk besser generalisiert!")
